...
account_sid = 'AC6e2521c986042c35204f4d91c48e0ba6'
auth_token = '013c354826ec0fa10ed6686487c3c89d'
client = Client(account_sid, auth_token)

def send_alert_message(to, body):
    message = client.messages.create(
        body=body,
        from_='+13159155136',  # Twilio 번호
        to='+821086438661'  # 받는 사람의 번호
    )
    return message.sid
                ...
                if len(faces) > 0:
                    for (x, y, w, h) in faces:
                        ...
                        if result[0][0] > 0.7:
                            ...
                            send_alert_message('+821086438661', '등록되지 않은 사람이 출입했습니다')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user['password'] == password:
                ...
                # 세션에 유저 정보 저장하기
                session['user'] = {'uid': user_id, 'email': user['email']}
                return redirect('/')
            else:
                return render_template('login.html', error="Invalid password.")
        except IndexError:
            return render_template('login.html', error="The email doesn't exist.")
        except Exception as e:
            return str(e)
    else:
